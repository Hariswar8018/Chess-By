...
class Send{
  ...
  static Future<void> sendNotificationsToAllUsers(String title, String message, String tournamentId) async {
    try {
      ...
      if (snapshot.docs.isEmpty) {
        print("No players found in the tournament.");
        return;
      }
      ...
      for (var doc in snapshot.docs) {
        ...
        if (token.isNotEmpty) {
          try {
            await sendNotificationsToTokens(title, message, token);
            print("Notification sent to $name (Doc ID: ${doc.id})");
          } catch (e) {
            print("Failed to send notification to $name (Doc ID: ${doc.id}): $e");
          }
        } else {
          print("No valid token for user: $name (Doc ID: ${doc.id})");
        }
      }
    } catch (e) {
